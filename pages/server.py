import plotly.graph_objects as go
import numpy as np
import asyncio

def update_graph(fig, arr, current_index=None, min_index=None):
    colors = ['blue'] * len(arr)
    if current_index is not None:
        colors[current_index] = 'red'
    if min_index is not None:
        colors[min_index] = 'green'
    fig.data[0].y = arr
    fig.data[0].marker.color = colors

async def bubble_vis(been_num, fig):
    been_num_val = int(been_num)
    arr = np.arange(1, been_num_val + 1, 1)
    np.random.shuffle(arr)
    fig.data[0].x = list(range(len(arr)))
    fig.data[0].y = arr
    
    for i in range(been_num_val):
        for j in range(0, been_num_val - i - 1):
            update_graph(fig, arr, j)
            await asyncio.sleep(0.6)  # Add a delay for better visualization
            if arr[j] > arr[j + 1]:
                arr[j], arr[j + 1] = arr[j + 1], arr[j]
                update_graph(fig, arr, j + 1)
                await asyncio.sleep(0.5)  # Add a delay for better visualization
        # Mark the last sorted bar with the normal color
        update_graph(fig, arr)

    # Final update to mark the array as fully sorted
    update_graph(fig, arr)

def generate_random_array(size):
    arr = np.arange(1, size + 1)
    np.random.shuffle(arr)
    return arr
